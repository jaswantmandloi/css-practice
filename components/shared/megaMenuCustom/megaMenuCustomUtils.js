...
export const useKeyboardNavigation = function({
    ...
    const mutationObserver = useCallback(function (event) {
    ...
    useEffect(() => {
        if (isActive && items.length && upPress) {
            cursorPosition.current = (cursorPosition.current > 0 ? cursorPosition.current - 1 : cursorPosition.current)
    ...
    useEffect(() => {
        if (isActive && items.length && downPress) {
            cursorPosition.current = cursorPosition.current < items.length - 1 ? cursorPosition.current + 1 : cursorPosition.current
            const {id:selectedId} = items[cursorPosition.current]
            setSelectedItemId(selectedId)
        }
