...
export default function MegaMenu() {
    ...
    useEffect(() => {
        const loopFocus = (event) => {
            ...
            if (event.shiftKey && menuSentinelStart.current === document.activeElement) {
    ...
    const handleMouseEnterNavMenu = (event) => {
       ...
       allItems.forEach(divItem => {
           ...
           const toggleState = divItem.dataset.index === event.currentTarget.dataset.index
  ...
  return <nav ref={navMenuRef}>
        ...
        <ul className={styles["nav-menu"]}>
          {Menus.map((menu, index) => {
              return <li key={index} className={styles["nav-item"]}
        ...
        {Menus.map((menu, index) => {
            ...
            return <div className={styles["sub-nav"]} data-index={index} >
                {menu.subMenus.map((subMenus, index) => {
                    return <ul key={index}  
                    className={styles["sub-nav-group"]}>
